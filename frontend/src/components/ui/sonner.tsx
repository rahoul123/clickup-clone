  ...
  return (
    <Sonner
      ...
      toastOptions={{
        classNames: {
          toast:
            "group toast relative mb-[10px] w-full max-w-[320px] rounded-[14px] border border-slate-300/90 bg-white/98 px-4 py-[14px] text-slate-900 shadow-[0_10px_28px_rgba(15,23,42,0.18)] backdrop-blur-[10px] dark:border-white/10 dark:bg-[#1e1e2e]/95 dark:text-slate-100 dark:shadow-[0_8px_32px_rgba(0,0,0,0.4)] before:absolute before:inset-y-0 before:left-0 before:w-[3px] before:rounded-l-[14px] before:bg-indigo-500 data-[type=success]:before:bg-emerald-500 data-[type=warning]:before:bg-amber-500 data-[type=error]:before:bg-rose-500 data-[state=open]:animate-in data-[state=open]:slide-in-from-right-6 data-[state=open]:fade-in-0 data-[state=closed]:animate-out data-[state=closed]:slide-out-to-right-6 data-[state=closed]:fade-out-0",
          title: "text-[14px] font-semibold leading-5 text-slate-900 dark:text-white",
          description: "mt-0.5 text-[12px] font-normal leading-4 text-slate-600 dark:text-[#9ca3af]",
          closeButton:
            "absolute right-2 top-2 rounded-md p-1 text-slate-500 hover:bg-slate-100 hover:text-slate-700 dark:text-slate-500 dark:hover:bg-white/10 dark:hover:text-slate-300",
